                    ...
                    try {
                        ...
                        for (int i = stopFlagSize - 1; i >= 0; i--) {
                            ...
                            if (stopFlag[i] == result[indexInResult]) {
                                logD("发现" + byteArray2Hex(stopFlag[i]) + "等于" + byteArray2Hex(result[indexInResult]));
                                shouldCallOnReceiveBytes = true;
                            } else {
                                logD("发现" + byteArray2Hex(stopFlag[i]) + "不等于" + byteArray2Hex(result[indexInResult]));
                                shouldCallOnReceiveBytes = false;
                            }
                        }
